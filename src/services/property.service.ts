  ...
  return {
    ...
    imageVariants: Array.isArray(raw.imageVariants)
      ? raw.imageVariants
          ...
          .map((variant) => ({
            id: variant.id,
            thumb: String(variant.thumb || variant.medium || variant.full || ''),
            medium: String(variant.medium || variant.full || variant.thumb || ''),
            full: String(variant.full || variant.medium || variant.thumb || ''),
