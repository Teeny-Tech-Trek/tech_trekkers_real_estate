...
export default function Avatars() {
  ...
  const handleCreateAgent = async () => {
    if (!formData.name || !formData.personality || !formData.description) {
      alert('Please fill in all required fields (Name, Personality, Description).');
      return;
    }
    try {
      await createAgent({
      ...
      setFormData({ name: '', personality: '', voice: '', avatar: '', description: '', avatarFile: null });
    } catch {
  ...
  const openAgentLeads = (agentId: string) => {
    navigate(`/leads?agentId=${encodeURIComponent(agentId)}`);
  };
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-44 bg-slate-800/95 backdrop-blur-xl border border-slate-700/50 rounded-xl shadow-2xl z-20 py-1.5 overflow-hidden">
                    <button
                      onClick={() => { setShowMenu(false); alert(`Viewing details for agent: ${agent.name}`); }}
                    ...
                    <button
                      onClick={() => { setShowMenu(false); alert(`Editing agent: ${agent.name}`); }}
                      className="w-full px-3.5 py-2 text-left text-sm text-slate-300 hover:bg-slate-700/50 flex items-center gap-2.5 transition-colors"
                    >
                      <Edit className="w-3.5 h-3.5" /> Edit Agent
                    </button>
                    <button
                      onClick={() => { setShowMenu(false); alert(`Settings for agent: ${agent.name}`); }}
                      className="w-full px-3.5 py-2 text-left text-sm text-slate-300 hover:bg-slate-700/50 flex items-center gap-2.5 transition-colors"
                    >
